...
// Adjusted by Bruno Herfst (2016) to checkout placed incopy content first
        ...
        if(app.selection.length > 0){
            switch(app.selection[0].constructor.name){
                ...
                default:
                    ...
                    checkoutStories(app.activeDocument);
                    myFindChangeByList(app.activeDocument);
            }
        } else {
    ...
    if(myResult == true){
        switch(myRangeButtons.selectedButton){
            case 0:
                ...
                checkoutStories(myObject);
                break;
            case 1:
                myObject = app.selection[0].parentStory;
                if(checkOutStory(myObject)){
                    checkedOut.push(myObject);
                }
                break;
            case 2:
                myObject = app.selection[0];
                if(checkOutStory(myObject.parentStory)){
                    checkedOut.push(myObject.parentStory);
                }
                break;
        }
        myDialog.destroy();
...
function checkInStories() {
    for (var i = 0; i < checkedOut.length; i++) {
       checkInStory(checkedOut[i]);
    }
}

function checkoutStories(doc) {
    for (var i = 0; i < doc.stories.length; i++) {
       if( checkOutStory(doc.stories[i]) ){
           checkedOut.push(doc.stories[i]);
       }
    }
}

function checkOutStory(s){
